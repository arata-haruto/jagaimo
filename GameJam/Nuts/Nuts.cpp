...
void NutsDraw(float camera_x, float camera_y)//スクリーン座標の取得
{
	...
	if (Is_Fever == false)
	{
		...
		gauge_color = GetColor(0, 255, 100); // 普段は緑色
	}
	else
	{
		...
		if (Nuts_active[pos_Item_No] == TRUE)//表示されているもの
		{
			...
			if (distance < player_radius + 16.0f) // 木の実の半径16px
			{
				...
				if (Is_Fever == false)//フィーバーではない時
				{
					...
					//フィーバー木の実を取ったか、20個集まったらフラグを立てる
					if (Image_rand[pos_Item_No] == 8)
					{
						triggerFever = true; // フィーバー木の実なら即発動！
					}
					...
					//フィーバー中
						if (triggerFever == true) 
						{
							...
							Nut_MAX_Pieces = 800; // フィーバー中は表示枠をドバッと増やす
							...
							for (int i = 0; i < 999; i++) {
								...
								if (i < Nut_MAX_Pieces)
								{
									...
									else 
									{
										...
										Nuts_active[i] = TRUE;
										Nuts_Scale[i] = 0.0f; // ゼロから拡大アニメーションさせる
										Image_rand[i] = (GetRand(3)) * 2; // 0, 2, 4, 6 のどれか(新鮮な実限定)
